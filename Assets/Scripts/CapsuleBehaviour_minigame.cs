...
public class CapsuleBehaviour_minigame : MonoBehaviour {
	...
	void Update () {
		...
		reader3.Close();
		float pitch_degree=pitch_m*180/Mathf.PI;
		float yaw_degree;
		if(az>ay)pitch_m = ay*(float)0.045*Mathf.PI/180;
		else pitch_m = (2800-az)*(float)0.045*Mathf.PI/180;
		roll_m = -ax*(float)0.045*Mathf.PI/180;
		...
		yaw_degree=-((gx1)*Mathf.Sin(roll_m)-(gy1)*Mathf.Sin(pitch_m)*Mathf.Cos(roll_m)-(gz1)*Mathf.Cos(pitch_m)*Mathf.Cos(roll_m))*(float)0.000025*180/Mathf.PI;
		yaw_m = yaw_m+yaw_degree*2;
		Vector3 temp;
		Vector3 mayumi;
		//mini_game
			temp.x = ay*(float)0.045;temp.y = yaw_m;temp.z = ax*(float)0.045+90;
			transform.eulerAngles = temp;
			if(az>0)pitch_degree=temp.x-pitch_degree;
			else if(az<=0)pitch_degree=pitch_degree-temp.x;
			mayumi.x=0;mayumi.y=(float)1.3;mayumi.z=(float)-2.6;
			transform.RotateAround(mayumi,Vector3.right,pitch_degree*(float)1.2);//pitch
			transform.RotateAround(mayumi,Vector3.up,yaw_degree);//yaw
		//if(Input.GetKey(KeyCode.Z)||Application.loadedLevel>5&&flag==1){
	...
	//private bool BgwWork = false;//いらない
	private bool DataTrsmt = false;
	//private string SndData;//使ってない
	...
	/*チェックサムの挿入に使用予定だった*/
	///*---------------------------*/
	////'数値を指定した桁数の文字列に変換する（符号桁分の指定あり）
	///*---------------------------*/
	//private string ChangeStrAsc(long BaseValue, int Length, bool Sign) 
	//{
	//    string Data;
	//    int i;
	//    long Div;
	...
	//        if( Temp2 < 10)
	//        {
	//            //Data = Data + Change1Str(Temp2);
	//        }
	//        else
	//        {
	//            Data = Data + "E";
	//        }
	//        Temp1 = Temp1 - (Temp2 * Div);
	//        Div = Div / 10;
	//    }
	//    return Data;
	//}
	...
	////'指定した文字列を数値に変換する
	///*---------------------------*/
	//private int ChangeAscStr(string Asc)
	//{
	//    int Length;
	//    int temp = 0;
	//    int i = 0;
	...
	//            temp = temp * 10;
	//        }
	//        //temp = temp + Change1Value(Asc.Substring(i, 1));
	//    }
	//    return temp;
	//}
	/*---------------------------*/
	...
	public int DllStart(string PortName, int BaudRate, int DataLength , int Parity, int SBits)
	{
		...
		/* イベント処理関数の追加 */
		//VBコード
		//AddHandler Rs.DataReceived, AddressOf SerialReceiveData
		//AddHandler Tm.Elapsed, AddressOf Tm_Tick
		//'AddHandler Bgw.DoWork, AddressOf datagetprocess
	...
	public int Dllend()
	{
		...
		/* イベント処理関数の削除 */
		//VBコード
		//RemoveHandler Rs.DataReceived, AddressOf SerialReceiveData;
