...
public class Controller : MonoBehaviour {
	...
	public static int mono_flag;
	public static int gui_set;
	public float pitch0;

	public static Vector3 temp;
	public static Vector3 mayumi;
	public static float pitch_degree;
	public static float yaw_degree;
	...
	void Update () {
		...
		if(Application.loadedLevelName=="maze"||Application.loadedLevelName=="tilt_ball_tutorial"||Application.loadedLevelName=="tilt_ball"){
			...
			yaw_degree=-((gx1)*Mathf.Sin(roll_m)-(gy1)*Mathf.Sin(pitch_m)*Mathf.Cos(roll_m)-(gz1)*Mathf.Cos(pitch_m)*Mathf.Cos(roll_m))*(float)0.00001*180/Mathf.PI;
			yaw_m = yaw_m+yaw_degree*2;

			pitch_degree=pitch-pitch_degree;
			if(Application.loadedLevel>14){mayumi.x=0;mayumi.y=(float)1.5;mayumi.z=(float)-12;}
			else{mayumi.x=0;mayumi.y=(float)1.5;mayumi.z=(float)-7;}
			transform.RotateAround(Controller.mayumi,Vector3.left,pitch_degree/10f);//pitch
			transform.RotateAround(Controller.mayumi,Vector3.down,yaw_degree*1.2f);//yaw
			yaw = yaw_m * 4;
		...
		if(fl1==0&&fl2==0&&fl3==0&&fl4==0&&fl5==0&&fr1==0&&fr2==0&&fr3==0&&fr4==0&&fr5==0)flag=0;
		//if(fl1>30&&fr1>30)capsule.renderer.material.color = Color.magenta;
		//if(fl1<30||fr1<30)capsule.renderer.material.color = Color.cyan;
		//motion data
	...
	void OnGUI(){
		...
		if(gui_set==1){
			...
			GUI.TextArea(new Rect(0+sw/7,10,sw/5,sh/8),"時間 :  " + timer.ToString(),style);
			style.normal.textColor=(Color.blue);
			GUI.TextArea(new Rect(sw-sw/3,10,sw/6,sh/8),"点数  : "+score.ToString(),style);
		}
